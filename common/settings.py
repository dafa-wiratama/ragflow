...
def init_settings():
    ...
    if STORAGE_IMPL_TYPE in ['AZURE_SPN', 'AZURE_SAS']:
        ...
        GCS['bucket'] = os.getenv('RAGFLOW__GCS__BUCKET')
        GCS['path'] = os.getenv('RAGFLOW__GCS__PATH', '/')

        if not GCS['bucket']:
            raise ValueError("GCS bucket name must be provided via environment variable 'RAGFLOW__GCS__BUCKET'")
